...
show "[Sala VIP]";
show "Olá S"+strsex("r.","ra.")+" "+strcharinfo(0)+"";
show "Deseja entrar na sala VIP ?";
next;
switch(select("- Sala VIP Normal","- Não, obrigad"+strsex("o","a")+"")){
...
show "Deseja entrar na sala VIP ?";
next;
switch(select("- Sala VIP Premium","- Sala VIP Normal","- Não, obrigad"+strsex("o","a")+"")){
...
if (getgmlevel() < 1) {
show "[Torpedo]";
show "Você não é vip saia desta sala !";
close2;
...
sc_start 30, 240000, 10; // --- Benção
skilleffect 34,0; // --- Benção
sc_start 32, 240000, 10; // --- AGI
skilleffect 29,0; // --- AGI
// --- BUFFS abaixo estão desabilitados tire as // para habilitar
sc_start 35, 60000, 5; 
skilleffect 66,0; 
...
if (getgmlevel() < 10) {
show "[Resetadora]";
show "Você não é vip não pode ficar aqui !";
close2;
warp "prontera",150,150;
end;
}
...
show "Olá ^0099FF"+strcharinfo(0)+"^000000";
show "Eu estou aqui para";
show "ajuda-l"+strsex("o","a")+" do que ";
show "você precisa ?";
next;
switch(select("- Salvar","- Abrir Armazém","- Reparar Equipamentos","- Serviço de Teletransporte","- Nada, obrigad"+strsex("o","a")+"")){
...
next;
switch(select("- Desejo ir à uma Cidade","- Desejo ir à outros lugares")){
...
show "Por favor selecione seu destino.";
next;
switch(select("- Calabouço de Einbech","- Calabouço de Magma","- Caverna de Comodo",
"- Caverna de Payon","- Esfinge","- Esgoto de Prontera","- Fábrica de Brinquedos","- Formigueiro Infernal",
"- Glast Heim","- Ilha Byalan","- Ilha das Tartarugas","- Labirinto de Tatames",
"- Mina de Carvão","- Navio Fantasma","- Pirâmide","- Praia das Focas",
"- Poringlândia","- Vila dos Orcs","- O Lago do Abismo","- Calabouço de Kunlun",
...
show "Caverna de Comodo? Poderia ser mais específico?";
next;
...
force_5-1,100,107,5	script	Cirurgião Famoso	121,{

if (getgmlevel() < 10) {
show "[Cirurgião]";
show "Você não é VIP não pode fazer cirurgia !";
close2;
warp "prontera",150,150;
end;
}
...
show "[Cirurgião]";
show "Olá ^C0C0C0"+strcharinfo(0)+"^000000, eu sou um Cirurgião muito famoso";
show "Eu trabalhava em um hospital há muito tempo, mas eu fui demitido por fazer";
show "Uma experiencia com mudança de sexo que deu errada em um paciente";
show "Agora eu ja estou mais experiente e não cometo mais erros";
next;
show "[Cirurgião]";
show "Então ^C0C0C0"+strcharinfo(0)+"^000000 você gostaria de mudar de sexo ?";
show "Se você escolher mudar de sexo suas habilidades serão resetadas !";
next;
switch(select("- Sim, eu desejo mudar de sexo !","- Não, estou feliz assim !")){

case 1:
show "[Cirurgião]";
show "Muito bem eu irei começar a cirurgia agora, deite-se na cama de cirurgia por favor";
...
case 2:
show "[Cirurgião]";
show "Muito bem se você mudar de ideia fale comigo !";
close;
}
}
...
next;
switch(select("- Sim, vamos logo !","- Não, obrigado")){

case 1:
show "[Porteiro MvP]";
show "Até logo e boa sorte !";
close2;
...
if (getgmlevel() <10) {
show "[Invocador]";
show "Olá  ^00FF00"+strcharinfo(0)+"^000000 eu sou invocador !";
show "Eu só trabalho para os playes vips, para adquirir vip";
show "acesse o site do nosso server www.seuserver.com.br";
close;
}

show "[Invocador]";
show "Olá  ^00FF00"+strcharinfo(0)+"^000000 eu sou o invocador.";
show "Eu posso invocar um monstro por $@zeny zenys, você só tem que me dar o ID";
show "do monstro que você deseja invocar, gostaria de invocar um monstro ?";
next;
...
if (Zeny < $@zeny) {
show "[Invocador]";
show "Você não tem a quantia certa de zenys";
close;
}

show "[Invocador]";
show "Ótimo, agora digite o ID do monstro que você deseja invocar !";
show "^00FF00ATENÇÃO !! se você me der o ID errado irá perder a quantia de zenys^000000";
next;
digit $ID$;
next;
show "[Invocador]";
...
if (getgmlevel() < 10) {
show "[Bruxa]";
show "Infelizmente você não é VIP !";
close2;
warp "prontera",150,150;
end;
}
...
show "[Bruxa]";
show "Lá estava eu contra todo o exército de RuneMidGard.";
show "Eles estavam todos me olhando com raiva e me falando";
show "para desistir e me entregar, mas eu estava confiante.";
next;
show "[Bruxa]";
show "Eu disse a eles que nunca iria desistir, então...";
show "Eles avançaram............";
show "Vieram em minha direção com suas armas e poderes.";
show "Mas eles não sabiam meu verdadeiro poder...";
show "De repente..........";
next;
show "[Bruxa]";
...
show "Você escutou tudo que eu disse ??";
next;
switch(select("- Você tem um poder especial ?","- Não, estou indo embora")){

Case 1:
show "[Bruxa]";
show "Eu tenho sim idai ?? Não é da sua conta.";
show "Vocês VIP's acham que são melhores do que eu.";
show "Até parece !!";
next;
switch(select("- Você pode me transformar em alguem monstro ?","- Você é louca")){

Case 1:
show "[Bruxa]";
show "... ¬¬";
show "Você não tem medo de mim !!!???";
show "..........";
next;
show "[Bruxa]";
show "Muito bem, se eu te transformar em um monstro você vai embora ?";
next;
switch(select("- Sim !","- A vai se danar !")){

case 1:
show "[Bruxa]";
show "OK, então digite o ID do monstro que você quer virar !";
digit $ID$;
next;
show "[Bruxa]";
show "Pronto !";
